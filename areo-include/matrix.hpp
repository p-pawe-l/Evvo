...
public:
    ...
    Matrix(const T* data, std::size_t rows, std::size_t cols)
        : matrix_(std::make_unique<T[]>(rows * cols)), rows_(rows), cols_(cols) {
    ...
    explicit Matrix(const std::vector<std::vector<double>>& collection)
        : Matrix(collection.begin(), collection.end(), collection.size(),
                 collection.front().size()) {}
    ...
    Matrix(const Matrix<T>& other) : Matrix(other.matrix_.get(), other.rows_, other.cols_) {}

    /** @brief Copy-assigns a matrix, deep-copying the underlying data. */
    Matrix& operator=(const Matrix<T>& other) { return *this = Matrix(other); }

    /** @brief Returns a mutable view over the underlying row-major element buffer. */
    [[nodiscard]] std::span<T> data() noexcept { return {matrix_.get(), rows_ * cols_}; }
    ...
    void negate() noexcept { std::ranges::transform(data(), matrix_.get(), std::negate<>{}); }
    ...
    Matrix& operator+=(const Matrix<T>& other) {
        std::ranges::transform(data(), other.data(), matrix_.get(), std::plus<>{});
        return *this;
    }
     ...
     */
    Matrix& operator+=(T x) {
        std::ranges::transform(data(), matrix_.get(), [x](T element) { return element + x; });
        return *this;
    }
     ...
     */
    Matrix& operator-=(const Matrix<T>& other) {
        std::ranges::transform(data(), other.data(), matrix_.get(), std::minus<>{});
        return *this;
    }
